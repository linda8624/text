...
/**
 ...
 @param customShadowImage 底部的自定义分割线图片
...
+ (void)ba_helperSetNaviBarBarTintColor:(UIColor *__nullable)barTintColor
                      ...
                      customShadowImage:(UIImage * __nullable)customShadowImage
                   navigationController:(UINavigationController *)navigationController
{
    ...
    if (!BAKit_ObjectIsNull(customShadowImage))
    {
        bar.shadowImage = customShadowImage;
        [bar setBackgroundImage:[UIImage ba_image_Color:BAKit_Color_Clear] forBarMetrics:UIBarMetricsDefault];
    }
    else
    {
        if (!isNeedBottomLine)
        {
            // 去掉导航分割线
            [navigationController ba_hideBottonLineInView:navigationController.navigationBar];
        }
        
        if (barTintColor)
        {
            bar.shadowImage = [UIImage ba_image_Color:BAKit_Color_Gray_8];
            [bar setBackgroundImage:[UIImage ba_image_Color:barTintColor] forBarMetrics:UIBarMetricsDefault];
        }
    }
}
